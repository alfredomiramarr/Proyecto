<?php
	...
	header("Content-Type: text/html; charset=ISO-8859-1", true);
		...
		$sql = "SELECT * FROM C_Spray WHERE Spray_Id = ".$id.";";
		$existe=0;
		//echo $sql;
		$result = mysqli_query($conexion,$sql);
		while($consulta = mysqli_fetch_array($result))
		  {
		    $id = $consulta['Spray_Id'];
			$nombre =$consulta['Spray_Name'];
            $descrip = $consulta['Spray_Desc'];
            $img = $consulta['Spray_Image'];
			/*session_start();
			ob_start();*/
			//$_SESSION['i'] = $id;
		    $existe++;
...
<head>
	...
	<header align="right" background="white">
		<style>
			...
			.body{
				font-family: 'Prompt', sans-serif;
				background-image: url('images/new_ddt.jpg');
		...
		<div class="tabla10 p-l-50 p-r-50 p-t-30 p-b-30">
			<form class="login100-form validate-form" method="post" enctype="multipart/form-data">
				...
				<div class="wrap-input10 validate-input m-b-25" >
					<input class="input100" type="text" name="n" placeholder="Nombre del insecticida" required="required" value="<?php echo $nombre;?>"/>
					<span class="focus-input100"></span>
				</div>
				<div class="wrap-input10 validate-input m-b-20" >
					<input class="input100" type="text" name="d" placeholder="Descripción" required="required" value="<?php echo $descrip;?>"/>
					<span class="focus-input100"></span>
				</div>	
				<p>Insertar imagen</p>		
				<div class="wrap-input10 validate-input m-b-25" >
...
if(isset($_POST['actualizar_btn']))
	{
		...
		if($id<>$a)
		{
			...
			$sqlverifica = "SELECT * FROM C_Spray WHERE Spray_Id = '$id'";
			$verificarSpray = mysqli_query($conexion,$sqlverifica);
			if (mysqli_num_rows($verificarSpray)>0)
			{
				echo $id;
				echo '<script>
				...
				$_UPDATE_SQL="UPDATE C_Spray Set
				  Spray_Name = '$nombre',
                  Spray_Desc = '$descrip',
				  Spray_Image = '$imgruta'
	              WHERE Spray_Id = $id";
		        ...
		        echo '<script>
			      alert("Spray actualizado con exito");
