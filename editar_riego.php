<?php
	...
	header("Content-Type: text/html; charset=ISO-8859-1", true);
		...
		$sql = "SELECT * FROM C_Irrigation WHERE Irrigation_ID = ".$id.";";
		$existe=0;
		//echo $sql;
		$result = mysqli_query($conexion,$sql);
		while($consulta = mysqli_fetch_array($result))
		  {
		    $id = $consulta['Irrigation_ID'];
			$frecc =$consulta['Frecuency'];
            $quant = $consulta['Quantity'];
...
<head>
	...
	<header align="right" background="white">
		<style>
			...
			.body{
				font-family: 'Prompt', sans-serif;
				background-image: url('images/new_riego.jpg');
		...
		<div class="wrap-login100 p-l-50 p-r-50 p-t-30 p-b-30">
			<form class="login100-form validate-form" method="post" enctype="multipart/form-data">
				...
				<div class="wrap-input10 validate-input m-b-25" >
					<input class="input100" type="text" name="f" placeholder="Frecuencia" required="required" value="<?php echo $frecc;?>"/>
					<span class="focus-input100"></span>
				</div>
				<div class="wrap-input10 validate-input m-b-20" >
					<input class="input100" type="number" name="q" placeholder="Cantidad de agua utilizada (litros)" required="required" value="<?php echo $quant;?>"/>
					<span class="focus-input100"></span>
				</div>	
				<div class="wrap-input10 validate-input m-b-20" >
					<input class="input100" type="text" name="d" placeholder="Descripci&#211n" required="required" value="<?php echo $desc;?>"/>
					<span class="focus-input100"></span>
				</div>	
				<p>Insertar un icono referente al sistema de riego registrado</p>		
				<div class="wrap-input10 validate-input m-b-25" >
...
if(isset($_POST['actualizar_btn']))
	{
		...
		if($id<>$a)
		{
			...
			$sqlverifica = "SELECT * FROM C_Irrigation WHERE Irrigation_ID = '$id'";
			$verificarRiego = mysqli_query($conexion,$sqlverifica);
			if (mysqli_num_rows($verificarRiego)>0)
			{
				echo $id;
				echo '<script>
				...
				$_UPDATE_SQL="UPDATE C_Irrigation Set
				  Frecuency = '$frecc',
                  Quantity = '$quant',
                  Description = '$desc',
				  Image='$imgruta'
		        ...
		        echo '<script>
			      alert("Riego actualizado con exito");
