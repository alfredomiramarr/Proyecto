<?php
	...
	header("Content-Type: text/html; charset=ISO-8859-1", true);
		...
		$sql = "SELECT * FROM C_Fertilizer WHERE Fertilizer_ID = ".$id.";";
		$existe=0;
		//echo $sql;
		$result = mysqli_query($conexion,$sql);
		while($consulta = mysqli_fetch_array($result))
		  {
		    $id = $consulta['Fertilizer_ID'];
			$nombre =$consulta['Name'];
            $descrip = $consulta['F_Desc'];
            $img = $consulta['Fertilizer_Img'];
			/*session_start();
			ob_start();*/
			//$_SESSION['i'] = $id;
		    $existe++;
...
<head>
	...
	<header align="right" background="white">
		<style>
			...
			.body{
				font-family: 'Prompt', sans-serif;
				background-image: url('images/new_abono.jpg');
		...
		<div class="wrap-login100 p-l-50 p-r-50 p-t-30 p-b-30">
			<form class="login100-form validate-form" method="post" enctype="multipart/form-data">
				...
				<div class="wrap-input10 validate-input m-b-25" >
					<input class="input100" type="text" name="n" placeholder="Nombre del fertilizante" required="required" value="<?php echo $nombre;?>"/>
					<span class="focus-input100"></span>
				</div>
				<div class="wrap-input10 validate-input m-b-20" >
					<input class="input100" type="text" name="des" placeholder="Descripción" required="required" value="<?php echo $descrip;?>"/>
					<span class="focus-input100"></span>
				</div>	
				<p>Insertar imagen</p>	
				<div class="wrap-input10 validate-input m-b-25" >
...
if(isset($_POST['actualizar_btn']))
	{
		...
		if($id<>$a)
		{
			...
			$sqlverifica = "SELECT * FROM C_Fertilizer WHERE Fertilizer_ID = '$id'";
			$verificarAbono = mysqli_query($conexion,$sqlverifica);
			if (mysqli_num_rows($verificarCultivo)>0)
			{
				...
				$_UPDATE_SQL="UPDATE C_Fertilizer Set
				  Name = '$name',
                  F_Desc = '$descrip',
				  Fertilizer_Img='$imgruta'
	              WHERE Fertilizer_ID = $id";
	              //move_uploaded_file($imgarchivo, $imgruta);
	            mysqli_query($conexion,$_UPDATE_SQL);
		        echo '<script>
			      alert("Fertilizante actualizado con exito");
