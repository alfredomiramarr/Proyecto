...
class plagasDAO {
	...
	function selectAll() {
		...
		while($fila = mysqli_fetch_assoc($resultado)) {
			$vo = new plagasVO();
			$vo->setAll($fila['Plague_ID'],$fila['Name'],$fila['Description'],$fila['Image'],$fila['Status']);
			$listadoVO[] = $vo;
		}
		if(!empty($listadoVO))
			return $listadoVO;
	...
	function Insert($vo) {
		$sql = "INSERT INTO C_Plague (Name, Description, Image) VALUES ('".$vo->getNombre()."','".$vo->getDescripcion()."','".$vo->getImagen()."')";
		$resultado=mysqli_query($this->conn,$sql);
		if($resultado)
			return true;
		else
