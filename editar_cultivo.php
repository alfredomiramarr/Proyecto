<?php
	...
	header("Content-Type: text/html; charset=ISO-8859-1", true);
		...
		$sql = "SELECT * FROM C_Crop WHERE Crop_ID = ".$id.";";
		$existe=0;
		//echo $sql;
		...
		while($consulta = mysqli_fetch_array($result))
		  {
		    $id = $consulta['Crop_ID'];
			$name =$consulta['Crop_Name'];
            $type = $consulta['Crop_Type'];
            $quant = $consulta['Crop_Quant'];
            $img = $consulta['Crop_Img'];
			/*session_start();
			ob_start();*/
			//$_SESSION['i'] = $id;
		    $existe++;
...
<head>
	...
	<header align="right" background="white">
		<style>
			...
			.body{
				font-family: 'Prompt', sans-serif;
				background-image: url('images/new_cultivo.jpg');
		...
		<div class="tabla10 p-l-50 p-r-50 p-t-30 p-b-30">
			<form class="login100-form validate-form" method="POST" enctype="multipart/form-data">
				...
				<div class="wrap-input10 validate-input m-b-20" >
					<input class="input100" type="text" name="n" placeholder="Nombre del cultivo" required="required" value="<?php echo $name;?>"/>
					<span class="focus-input100"></span>
				</div>
				<div class="wrap-input10 validate-input m-b-20" >
					<input class="input100" type="text" name="t" placeholder="Tipo de cultivo" required="required" value="<?php echo $type;?>"/>
					<span class="focus-input100"></span>
				</div>

				<div class="wrap-input10 validate-input m-b-25" >
					<input class="input100" type="number" name="q" placeholder="Cantidad Existencia" required="required" value="<?php echo $quant;?>"/>
					<span class="focus-input100"></span>
				</div>
				<p>Insertar imagen</p>	
				<div class="wrap-input10 validate-input m-b-25" >
...
if(isset($_POST['actualizar_btn']))
	{
		...
		if($id<>$a)
		{
			...
			$sqlverifica = "SELECT * FROM C_Crop WHERE Crop_ID = '$id'";
			$verificarCultivo = mysqli_query($conexion,$sqlverifica);
			if (mysqli_num_rows($verificarCultivo)>0)
			{
				echo $id;
				echo '<script>
		        ...
		        echo '<script>
			      alert("Cultivo actualizado con exito");
